...
| 单实例防重复 | .lock 文件防止多开，进程退出自动清理 |
| 启动网络就绪检测 | 开机启动时等待网络就绪再进入主循环，避免误报 |
| 开机自启 | 支持安装为 Windows 计划任务，后台静默运行 |
| 日志记录 | 控制台彩色输出 + auth_log.txt 文件日志（UTF-8 编码） |

## 运行流程

```
启动 → 加载配置 → 单实例检测 → 等待网络就绪(最多60秒)
  ↓
主循环 (每5秒)
  ├─ 第1层: Ping 检测
  │   → 失败 = 断网，等待恢复后自动认证
...
> 日志文件为 UTF-8 编码，Windows 记事本可直接打开。
